...
def dropedge_and_normlize(sparse_tensor,dropout_rate,identity_matrix=None):

    """Row-normalize a sparse PyTorch tensor"""
    ...
    num_edges_to_drop = int(num_edges * dropout_rate)
    keep_mask = torch.ones(num_edges, dtype=torch.bool, device= 'cuda' if args.use_cuda  else 'cpu')
    drop_indices = np.random.choice(num_edges, num_edges_to_drop, replace=False)
    keep_mask[drop_indices] = False
    new_indices = indices[:, keep_mask]
    new_values = values[keep_mask]
    sparse_tensor = torch.sparse_coo_tensor(new_indices, new_values, sparse_tensor.size()).coalesce()
    # add identity_matrix
    sparse_tensor = (sparse_tensor + identity_matrix).coalesce()

    # Calculate row sums
    row_sum = torch.sparse.sum(sparse_tensor, dim=1).to_dense()
    row_sum_inv = 1.0 / row_sum
    row_sum_inv[torch.isinf(row_sum_inv)] = 0.0
    
    # Create a diagonal matrix of the row sum inverses
    row_indices = torch.arange(row_sum_inv.size(0), device= 'cuda' if args.use_cuda  else 'cpu')
    row_sum_inv_diag = torch.sparse_coo_tensor(
        indices=torch.stack([row_indices, row_indices]),
        values=row_sum_inv,
        size=(row_sum_inv.size(0), row_sum_inv.size(0))
    ).coalesce()
    
    # Normalize the sparse tensor by multiplying with the diagonal matrix
    normalized_sparse_tensor = torch.sparse.mm(row_sum_inv_diag, sparse_tensor)
    
    return normalized_sparse_tensor , normalized_sparse_tensor.to_dense()
...
def load_data_pretrain(dataset_source):
    ...
    if dataset_source in valid_num_dic.keys():
        ...
        identity_matrix = sparse_mx_to_torch_sparse_tensor(sp.eye(adj.shape[0]))
        ...
        #adj = adj.tocoo() + sp.eye(adj.shape[0])
        adj= sparse_mx_to_torch_sparse_tensor(adj)
        identity_matrix = sparse_mx_to_torch_sparse_tensor(sp.eye(adj.shape[0]))
    ...
    return adj, features, labels, idx_train, idx_valid, idx_test, n1s, n2s, class_train_dict, class_test_dict, class_valid_dict,identity_matrix
...
def pre_train(epoch,adj, adj_sparse, N, mode='train',cur=False,identity_matrix=None):
    ...
    # drop edge
    if cur==True: 
        c = update_competency(epoch+1, args.epochs, c_0, 2)
        #print('competency:',c)
        adj_sparse, adj = dropedge_and_normlize(adj_sparse,dropout_rate=c,identity_matrix=identity_matrix)
    ...
    for j in range(Q):
        ...
        class_contras_loss, similarity=InforNCE_Loss(target_embs[j],class_ego_embs,tau=0.5)
...
parser.add_argument('--data', type=str, default='cora-full',help='[cora-full,dblp,Amazon_eletronics,ogbn-arxiv]')
...
for dataset in [args.data]:
    ...
    adj_sparse, features, labels, idx_train, idx_val, idx_test, n1s, n2s, class_train_dict, class_test_dict, class_valid_dict, identity_matrix = load_data_pretrain(
        dataset)

    print('done')
    print(dataset)

    '''add self loop in scipy matrix'''
    adj_sparse_with_self_loops = adj_sparse + identity_matrix
    '''normlize'''
    sparse_tensor = adj_sparse_with_self_loops.coalesce()
    # Calculate row sums
    row_sum = torch.sparse.sum(sparse_tensor, dim=1).to_dense()
    row_sum_inv = 1.0 / row_sum
    row_sum_inv[torch.isinf(row_sum_inv)] = 0.0
    # Create a diagonal matrix of the row sum inverses
    row_indices = torch.arange(row_sum_inv.size(0), device= 'cpu')
    row_sum_inv_diag = torch.sparse_coo_tensor(
        indices=torch.stack([row_indices, row_indices]),
        values=row_sum_inv,
        size=(row_sum_inv.size(0), row_sum_inv.size(0))
    ...
    # Normalize the sparse tensor by multiplying with the diagonal matrix
    adj_sparse_normlize = torch.sparse.mm(row_sum_inv_diag, sparse_tensor)

    '''to dense'''
    adj = adj_sparse.to_dense()
    adj_normlize = adj_sparse_normlize.to_dense()
        ...
        for K in K_set:
            for repeat in range(5):
                ...
                if args.use_cuda:
                    ...
                    adj_normlize = adj_normlize.cuda()
                    adj_sparse_normlize = adj_sparse_normlize.cuda()
                ...
                for epoch in range(args.epochs):
                    ...
                    #t=time.time()
                    acc_train=pre_train(epoch,adj_normlize,adj_sparse_normlize, N=N,cur=False)
                    if  epoch > 0 and epoch % 50 == 0:
                        temp_accs=[]
                        for epoch_test in range(20):
                            temp_accs.append(pre_train(epoch_test,adj_normlize,adj_sparse_normlize, N=N, mode='test'))
                        accs = []
                        for epoch_test in range(20):
                            accs.append(pre_train(epoch_test,adj_normlize,adj_sparse_normlize, N=N if dataset!='ogbn-arxiv' else 5, mode='valid'))
                ...
                print('One Stage Test Acc:',np.array(ori_accs).mean(axis=0))
                print('--------time :',time.time()-t_total,'s-----------------')
                ...
                for epoch in range(args.epochs):
                    #t=time.time()
                    acc_train=pre_train(epoch,adj,adj_sparse, N=N,cur=True,identity_matrix=identity_matrix)
                    if  epoch > 0 and epoch % 50 == 0:
                        temp_accs=[]
                        for epoch_test in range(20):
                            temp_accs.append(pre_train(epoch_test,adj_normlize,adj_sparse_normlize, N=N , mode='test'))
                ...
                accs=best_test_accs
                print('Two Stage Test Acc:',np.array(accs).mean(axis=0))
                print('--------time :',time.time()-t_total,'s-----------------')

                results[dataset]['{}-way {}-shot {}-repeat'.format(N,K,repeat)]=[np.array(ori_accs).mean(axis=0),np.array(accs).mean(axis=0)]
                json.dump(results[dataset],open('./{}cmp-TENT-result_{}.json'.format(args.f,dataset),'w'))

            accs_ori=[]
            accs=[]
            for repeat in range(5):
                accs_ori.append(results[dataset]['{}-way {}-shot {}-repeat'.format(N,K,repeat)][0])
                accs.append(results[dataset]['{}-way {}-shot {}-repeat'.format(N,K,repeat)][1])

                

            results[dataset]['{}-way {}-shot'.format(N,K)]=[np.mean(accs_ori),np.mean(accs)]
            results[dataset]['{}-way {}-shot_print'.format(N,K)]=['ori_acc: {:.4f}'.format(np.mean(accs_ori)),'acc: {:.4f}'.format(np.mean(accs))]
